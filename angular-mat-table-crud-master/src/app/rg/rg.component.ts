...
export class RgComponent implements OnInit {
  displayedColumns = ['name', 'subscription_id', 'location' ,'actions'];
  ...
  deleteItem(i: number, subscription_id: string, name: string,  location: string,event:any) {
    console.log("event=========="+event);
    this.index = i;
    this.id = name;
    const dialogRef = this.dialog.open(DeleteRgDialogComponent, {
      data: {i:i,subscription_id: subscription_id, name: name,  location: location}
    });
...
export class ExampleDataSource extends DataSource<Subscribers> {
  ...
  connect(): Observable<Subscribers[]> {
    ...
    this._exampleDatabase.getRg();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.data.slice().filter((issue: Subscribers) => {
        this.loaderService.hideLoader();
        const searchStr = (issue.subscription_id+issue.name+issue.location).toLowerCase();

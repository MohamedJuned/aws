...
export class SaComponent implements OnInit {
  displayedColumns = ['name', 'type','kind','resource_group_name','location', 'subscription_id','actions'];
  ...
  startEdit(i: number, subscription_id: string, azure_tenant_id: string, application_id: string, azure_Key: string) {
    this.id = subscription_id;
    // index row is used just for debugging proposes and can be removed
    //this.index = i;
    console.log(subscription_id);
    const dialogRef = this.dialog.open(EditDialogComponent, {
      data: {i:i,subscription_id: subscription_id, azure_tenant_id: azure_tenant_id, application_id: application_id, azure_Key: azure_Key}
    });
  ...
  deleteItem(i: number, name: string, type: string, kind: string,resource_group: string,location: string, subscription_id: string) {
    this.index = i;
    this.id = name;
    const dialogRef = this.dialog.open(DeleteSaDialogComponent, {
      data: {i: i, name: name, type: type, kind: kind,resource_group: resource_group,location: location, subscription_id: subscription_id}
    });
...
export class ExampleDataSource extends DataSource<Subscribers> {
  ...
  connect(): Observable<Subscribers[]> {
    ...
    this._exampleDatabase.getSa();

    return Observable.merge(...displayDataChanges).map(() => {
      // Filter data
      this.filteredData = this._exampleDatabase.data.slice().filter((issue: Subscribers) => {
        this.loaderService.hideLoader();
        const searchStr = (issue.subscription_id+issue.username).toLowerCase();

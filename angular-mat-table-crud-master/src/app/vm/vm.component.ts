...
export class VmComponent implements OnInit {
  displayedColumns = ['vm_name', 'type', 'status', 'resource_group_name','location','subscription_id','actions'];
  ...
  startEdit(i: number, subscriptionid: string, vm_name: string, resource_group_name: string, location: string,status:string) {
    ...
    const dialogRef = this.dialog.open(EditDialogComponent, {
      data: {i:i,subscriptionid: subscriptionid, vm_name: vm_name, resource_group_name: resource_group_name, location: location, status:status}
    });
  ...
  stopServer(i: number, subscriptionid: string, vm_name: string, resource_group_name: string, location: string,status:string) {
    this.id = subscriptionid;
    // index row is used just for debugging proposes and can be removed
    //this.index = i;
  ...
  deleteItem(i: number, subscriptionid: string, vm_name: string, resource_group_name: string, location: string,status:string) {
    this.index = i;
    this.id = vm_name;
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result === 1) {
        const foundIndex = this.exampleDatabase.dataChange.value.findIndex(x => x.vm_name === this.id);
